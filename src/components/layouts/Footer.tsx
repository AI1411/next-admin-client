...
const link_list: link_list_type[] = [
  {
    name: 'github',
    link: 'https://github.com/AI1411',
  ...
  {
    name: 'facebook',
    link: 'https://www.facebook.com/akira.ishii.393/',
    icon:   <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path fillRule="evenodd"
    ...
    <div>
      <footer
        ...
        <div className="flex sm:justify-center space-x-6">
          {link_list.map((item, index) =>
            <a key={item.name} href={item.link} className="text-gray-500 hover:text-gray-900" target={`_blank`}>
              {item.icon}
            </a>
          )}
        </div>
      ...
      <p className="text-center text-sm text-gray-500 my-10">
        &copy; 2019-2021 <a href="https://themesberg.com" className="hover:underline"
                            target="_blank">AI1411</a>. All rights reserved.
      </p>
    </div>
  );
};
